...
test('Check loads and displays text', () => {
  const renderResult = render(<Draggable>Some text</Draggable>);
...
test('Check custom tagName', () => {
  const renderResult = render(<Draggable as="strong">Some text</Draggable>);

  expect(renderResult.getByText('Some text').tagName).toBe('STRONG');
});

test('Check className', () => {
  const renderResult = render(<Draggable className="box">Some text</Draggable>);

  expect(renderResult.getByText('Some text').classList.contains('box')).toBeDefined();
});

test('Check test id attribute', () => {
  const renderResult = render(<Draggable data-testid="some value" />);
